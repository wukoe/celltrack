...
def read_coords_from_csv(csv):
    ...
    for it in group_obj:
        ...
        frame_coord.append(it[1][['Y','X']].to_numpy())
    return cellid,frame_coord

def takein_folder_image(folder_path, ftype='*.tif'):
    """
    take in a folder's (TIFF) images; combine to one tensor; store it.
    """
    fl = glob.glob(os.path.join(folder_path, ftype))
    fl.sort()
    print('{} files in total'.format(len(fl)))

    D = []
    for it in fl:
        x = file_misc.imgtiff_read(it)
        D.append(x)
    D = np.stack(D)
    print(D.shape)

    fp,fname = folder_path.rstrip('/').rsplit('/', 1) #存储到上一级目录下。
...
def maskoverimg(frame, imobj, show_id=False):
    ...
    imobj : 可以是3种类型，same size as frame.
    '''
    # all to masks format.
    ...
    if show_id:
        ...
        img = show.overlay_text(img, mc, id_text, font_size=20)
    return img

def overlay_all(vid, images, show_id=False):
    fnum = len(vid)
    assert len(images) == fnum, "img mask number not same"
    ovimg = [] 
    for k in range(fnum):
        ovimg.append(maskoverimg(vid[k], images[k], show_id=show_id))
...
def colorize_mask(pred_mask):
    ...
    save_mask.putpalette(_palette)   #>>> # save_mask.save(os.path.join(output_dir, file_name))
    save_mask = save_mask.convert(mode='RGB')
    return np.array(save_mask)

def save_prediction(pred_mask, output_dir, file_name, opt='direct', image=None, save_array=False):
    ...
    if opt=='overlay':
        ...
        ovimg = colorize_mask(pred_mask)
    file_misc.imwrite(os.path.join(output_dir, file_name), ovimg)

    if save_array:
        fname = file_name.rsplit('.', 1)[0]
        # fname += '.pk'
        fm.pdump(os.path.join(output_dir, fname), pred_mask) #【】
    
def all2imgfile(maps, output_dir='/home/wb/samba_dir/indev/imouts/', fname='', direct_write=False):
    # 保存多个map
...
if __name__ == "__main__":
    # process original image data (for segmentation and tracking)
    takein_folder_image('/home/wb/samba_dir/cells_brightfield/single-cell-movement-for-machine-learning/HELA/1/B1ROI1')
